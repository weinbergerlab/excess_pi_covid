...
plot( res.pi.nat$obs/ res.pi.nat$pred, type='l')
abline(h=1)
```
...
covid.deaths.nat <- aggregate(cd1[,"covid.death.hybrid",drop=F], by=list('week_end'=cd1$week_end), FUN=sum)
...
plot(nat.range.ac2$week_end, rat1, type='l')
```
...
stack_plot_func(nat.range.ac2.spl[['NY']])
title("New York (exclude NYC)")

stack_plot_func(nat.range.ac2.spl[['NYC']], legend=F)
title("New York City")
...
stack_plot_func(nat.range.ac2.spl[['NJ']], legend=F)
title("New Jersey")

stack_plot_func(nat.range.ac2.spl[['IL']], legend=F)
title("Illinois")

stack_plot_func(nat.range.ac2.spl[['MI']], legend=T)
title("Michigan")

stack_plot_func(nat.range.ac2.spl[['MA']], legend=F)
title("Massachusetts")
...
## Table S2: Adjust for flu: all-cause
...
names(summary.table.adj.adj) <- c('State', 'Excess deaths, unadjusted for influenza','Excess deaths, adjusted for influenza' )
...
nat1 <- analysis.data2[analysis.data2$state=='FL',]
...
mod5 <- glm.nb(fit.ac~ sin52 +cos52 +sin26 +cos26 +epiyr +log.flu.deseasonalize +offset(log.offset), data=nat1)
pred5 <- predict(mod5, type='response', newdata=nat1)

nat1$log.offset <- log(nat1$prop.complete.offset+0.5)
mod6 <- glm.nb(fit.ac~ sin52 +cos52 +sin26 +cos26 +epiyr +offset(log.offset), data=nat1)

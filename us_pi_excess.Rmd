...
## Estimated excess deaths across the available states, by week
```{r}
## excess deaths
...
ts.plot.func(ds.plot=jh3, death.var='covid.track.death' ,states.plot=unique(jh3$state),ylim.adj=1.5 )
...
## Supplementary analysis 1: Excess deaths by week across the US, compared with official CDC numbers
```{r}
count.excess <-jh3[jh3$date >= as.Date(count.start.date) & jh3$date <=max(pi.data$week_end) ,]
...
htmlTable(count.excess4, caption=paste0('Observed and Excess deaths due to pneumonia & influenza, and COVID-19, from February 9, 2020 through ', last.date.format), rnames=F)
```
...
```
## Table S2: Excess all-cause deaths by state
```{r}

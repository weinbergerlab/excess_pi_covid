...
#  
#  url1 <- paste0('https://www.cdc.gov/coronavirus/2019-ncov/covid-data/covidview/',fri.date, '/csv/nchs-mortality-report.csv')
#  
# if(url.exists(url1)){
#  download.file(url1,'./Data/pic_provisional/pic.provisional.txt')
# }else{
#   fri.dat2 <-"04172020"
#   url2 <- paste0('https://www.cdc.gov/coronavirus/2019-ncov/covid-data/covidview/',fri.dat2, '/csv/nchs-mortality-report.csv')
#    download.file(url2,'./Data/pic_provisional/pic.provisional.txt')
# 
# }
#   
...
cdc.data$start_week <-
  as.Date(substr(cdc.data$start_week,1,10))
...
cdc.summary.nat$state <- NULL

cdc.summary.wk <- cdc.summary.nat

cdc.summary.wk <- cdc.summary.wk[, c("start_week", 'end_week' ,'nchs.covid.deaths','nchs.total.deaths','percent_complete', 'nchs.pneu.death','nchs.pneum.w.covid','nchs.flu.death','nchs.pic')]
...
nchs_covidview <- 
  nchs_covidview[cdc.summary.wk$end_week < 
                   max(cdc.summary.wk$end_week),]

plot(nchs_covidview$end_week, nchs_covidview$nchs.total.deaths)
...
natl.all.data.set <- natl.all.data
...
mmwr.wk1 <- MMWRweek(nchs_covidview$end_week)
nchs_covidview$year_week_num <- mmwr.wk1$MMWRweek
#nchs_covidview <- nchs_covidview[,c('end_week',"nchs.covid.deaths", "nchs.total.deaths" ,  "nchs.pneu.death" , 'year_week_num')]
...
nchs_covidview$year_week_num <- NULL

nchs_covidview$start_week <- NULL

natl.all.data <-
  merge(natl.all.data,
        nchs_covidview,
        by.x='week_end', by.y='end_week',all=T)

#Use 'all-death' column from covid view since this is updated daily
natl.all.data$all_deaths[!is.na(natl.all.data$nchs.total.deaths)] <- 
  natl.all.data$nchs.total.deaths[!is.na(natl.all.data$nchs.total.deaths)]

# natl.all.data <-
#   merge(natl.all.data, pic1, by.x=c('year','year_week_num'),by.y=c('year','week') ,all=T)

natl.all.data$nchs.pic[is.na(natl.all.data$nchs.pic)] <-
  natl.all.data$total_pni[is.na(natl.all.data$nchs.pic)]
...
agg.historic <- aggregate(natl.all.data.sub[,c("all_deaths","total_pni", "nchs.covid.deaths")], by=list('year'=natl.all.data.sub$year), FUN=sum)
...
```{r check.stata.data.agg}
#Run some reality checks on the data
natl.all.data.set$year <- 
  year(natl.all.data.set$week_end)

natl.all.data.set$year_week_num <- as.numeric(as.character(natl.all.data.set$year_week_num))

natl.all.data.set$epiyear <- 
  natl.all.data.set$year

natl.all.data.set$epiyear[natl.all.data.set$year_week_num <= 26] <- 
  natl.all.data.set$epiyear[natl.all.data.set$year_week_num <= 26] -1

nat.m <- melt(natl.all.data.set[,c("all_deaths",'epiyear','year_week_num')], id.vars=c('epiyear','year_week_num'))
nat.c <- dcast( nat.m, year_week_num~epiyear, fun.aggregate = sum)

nat.c <- nat.c[,c('year_week_num','2015','2016','2017','2018','2019')]

cols1<- c(rep('gray',4),'red')
matplot(nat.c[nat.c$year_week_num<=15,-1], type='l', col=cols1, bty='l', ylab='N deaths')

#nat.c$ave.hist <- (nat.c$`2016`+nat.c$`2017`+nat.c$`2018`)/3

diffs <-nat.c
diffs$diff2018 <- diffs$`2019` - diffs$`2018`
diffs$diff2017 <- diffs$`2019` - diffs$`2017`
diffs$diff2016 <- diffs$`2019` - diffs$`2016`
diffs$diff2015 <- diffs$`2019` - diffs$`2015`
diffs <- diffs[diffs$year_week_num<=15,]
plot(diffs$year_week_num, diffs$diff2015, type='l', col='gray', ylim=c(-5000, 8000), bty='l', ylab='Difference between 2020 and previous 4 years')
points(diffs$year_week_num, diffs$diff2016, type='l', col='gray')
points(diffs$year_week_num, diffs$diff2017, type='l', col='gray')
points(diffs$year_week_num, diffs$diff2018, type='l', col='gray')
abline(h=0, lty=2)


nat.c.epiwk <- nat.c
nat.c.epiwk$epiwk.num <- nat.c.epiwk$year_week_num
nat.c.epiwk$epiwk.num[nat.c.epiwk$year_week_num<=26] <- 
nat.c.epiwk$year_week_num[nat.c.epiwk$year_week_num<=26]+26
nat.c.epiwk$epiwk.num[nat.c.epiwk$year_week_num>26] <- 
nat.c.epiwk$year_week_num[nat.c.epiwk$year_week_num>26]-26
nat.c.epiwk <- nat.c.epiwk[nat.c.epiwk$year_week_num<=52,]
#matplot(nat.c.epiwk$epiwk.num, nat.c.epiwk[,3:6], type='l')

diffs.epiwk <-nat.c.epiwk
diffs.epiwk$`2019`[diffs.epiwk$`2019`==0] <- NA
diffs.epiwk$diff2018 <- diffs.epiwk$`2019` -
  diffs.epiwk$`2018`
diffs.epiwk$diff2017 <- diffs.epiwk$`2019` -
  diffs.epiwk$`2017`
diffs.epiwk$diff2016 <- diffs.epiwk$`2019` -
  diffs.epiwk$`2016`
diffs.epiwk$diff2015 <- diffs.epiwk$`2019` -
  diffs.epiwk$`2015`
#diffs.epiwk <- diffs.epiwk[diffs.epiwk$year_week_num<=15,]
plot(diffs.epiwk$epiwk.num, diffs.epiwk$diff2018, type='l',ylim=c(-5000, 15000), main='2019/20 minus prev. years', xaxt='n')
axis(side=1, at=diffs.epiwk$epiwk[c(1,5,9,13,17,21,24,28,32,36,40,44,48,52)], labels=diffs.epiwk$year_week_num[c(1,5,9,13,17,21,24,28,32,36,40,44,48,52)])
points(diffs.epiwk$epiwk.num, diffs.epiwk$diff2017, type='l')
points(diffs.epiwk$epiwk.num, diffs.epiwk$diff2016, type='l')
abline(h=0)
```
...
#str(cdc.summary.state)

pi.data <- 
  merge(pi.data,cdc.summary.state[,c('start_week','end_week','state','nchs.covid.deaths','nchs.total.deaths','nchs.pneu.death','nchs.pneum.w.covid','nchs.flu.death','nchs.pic')], by.x=c('region_name',"week_end"), by.y=c('state','end_week'), all=T)

pi.data$percent_complete[pi.data$percent_complete > 1] <- 1

pi.data$state <-
  state.abb[match(pi.data$region_name, state.name)]
...
pi.data.agg <- aggregate(pi.data[,c('total_pni','all_deaths','pct.comp.wgt','nchs.covid.deaths','nchs.total.deaths','nchs.pneu.death','nchs.pneum.w.covid','nchs.flu.death','nchs.pic')], by=list('state'=pi.data$state_region, 'week_start'=pi.data$week_start, 'week_end'=pi.data$week_end), FUN=sum, na.rm=T)
...
pi.data.clean2$nchs.total.deaths[pi.data.clean2$week_end < as.Date('2020-02-01')] <-
  pi.data.clean2$all_deaths[pi.data.clean2$week_end < as.Date('2020-02-01')]

pi.data.clean2$nchs.pic[pi.data.clean2$week_end < as.Date('2020-02-01')] <-
  pi.data.clean2$total_pni[pi.data.clean2$week_end < as.Date('2020-02-01')]
...
res.pi <- res.pi1[res.pi1$week_end<=as.Date('2020-04-11'),]

write.csv(res.pi, './outputs/p_i_natl_obs_expected.csv')

sum(res.pi$unexplained.cases[res.pi$mmwr_week>=10 &
                               res.pi$mmwr_week<=15 & res.pi$week_end>=as.Date('2020-01-01')])
```
...
res.ac <- res.ac1[res.ac1$week_end<=as.Date('2020-04-11'),]

yrange.pneu <- range(c(res.ac$pred, res.ac$obs, res.ac$upi,0))

sum(res.ac$unexplained.cases[res.ac$mmwr_week>=10 & res.ac$mmwr_week<=15 & res.ac$week_end>=as.Date('2020-01-01')])
...
formatted.ac <- 
  paste0(round(sum.excess.deaths.range.ac[2],-2),
                      '(' ,
                      round(sum.excess.deaths.range.ac[1],-2),',',
                      round(sum.excess.deaths.range.ac[3],-2), ')'
                      )

quantile(sum.ac.excess, probs=c(0.025,0.1, 0.30, 0.5, 0.9, 0.7, 0.975))
...
comb2 <- merge(comb1,
               natl.all.data2[,c('week_end',"nchs.covid.deaths")],by.x='week_end_date', by.y='week_end')
...
syndrome.select <- 'nchs.pic' #which syndrome do you want to plot?
...
jh3 <- merge( jh3, pi.data.clean2[c('state', "week_start",'percent_complete',"nchs.covid.deaths")], by.x=c('state','date') , by.y=c('state',"week_start"), all=T)
...
jh3 <- jh3[ jh3$date <= max(pi.data$"week_start" +weeks(1), na.rm=T),]
jh3 <- jh3[jh3$state %in% states.cdc,]
...
ts.plot.func(ds.plot=jh3, death.var='nchs.covid.deaths' ,states.plot=unique(jh3$state),ylim.adj=2.0 )
...
dates.plot4 <- seq.Date(from=date.start[1], length.out=52, by='week')
...
pi.data.ny.separate$nchs.total.deaths[pi.data.ny.separate$week_end<as.Date('2020-02-01')] <-
  pi.data.ny.separate$all_deaths[pi.data.ny.separate$week_end<as.Date('2020-02-01')]

pi.data.agg.ny <- aggregate(pi.data.ny.separate[,c("nchs.pic","nchs.total.deaths" ,'pct.comp.wgt')], by=list('state'=pi.data.ny.separate$state_region, 'week_start'=pi.data.ny.separate$week_start, 'week_end'=pi.data.ny.separate$week_end), FUN=sum, na.rm=T)
pi.data.agg.ny$percent_complete <- pi.data.agg.ny$pct.comp.wgt/pi.data.agg.ny$nchs.total.deaths

spl1.ny<-split(pi.data.agg.ny, pi.data.agg.ny$state)
min.state.ny <- lapply(spl1.ny, function(x){ x$miss.x<-min(x$nchs.pic, na.rm=T)
return(x)
                })
...
syndrome.select <- 'nchs.total.deaths'
...
jh3a <- jh3a[,c('state','week_start',"nchs.covid.deaths" ,"covid.track.death","test.week.per.capita",'testN' )]
...
end.summary <- ac.summary.state3[ac.summary.state3$year==2020 & ac.summary.state3$week>=10 & ac.summary.state3$week<=15 ,]

agg.est <- 
  aggregate(end.summary[,c('excess_pi_deaths',"excess_all_cause_deaths","all_cause_deaths","nchs.covid.deaths" ,'covid.track.death')], by=list('state'=end.summary$state), FUN=sum) 
agg.est$ratio <-
...
#
report.ny <- jh3[jh3$state=='NY' & jh3$date %in% dates1.ac[dates.select], ]
...
points(report.ny$date,report.ny$nchs.covid.deaths, col='blue', type='l', lty=3)
 legend(as.Date('2020-02-09'), 6000, legend=c("Excess all-cause deaths",   "Excess P&I deaths", 'Reported COVID-19', 'NCHS COVID-19 deaths'),
       col=c("black", "red", 'blue', 'blue'), lty=c(1,1,2,3), cex=0.8) 
...
Fold difference between excess all-cause mortality and excess P&I&C mortality in NJ and NY. Shows that excess P&I accounts for ~50-60% of the excess deaths
```{r}
...
nyc.data <- 
  merge(nyc.data, pi.data[pi.data$region_name=='New York City',c('total_pni','all_deaths','week_end')], by='week_end', all=T)

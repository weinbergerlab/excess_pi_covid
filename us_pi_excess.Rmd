...
## Analysis #1: Simple adjustment for under-reporting

It is tempting to compare the weekly number of deaths in 2020 with the number of deaths during the same weeks in previous years. However, many states have extreme reporting delays, so data from the last several weeks or longer are highly incomplete. Therefore, it might look like deaths are actually going down, but they are stable. A simple analysis then is to take the provisional death report from the most recent week (which is incomplete for recent weeks) and compare this to the provisional report from the same week in 2019. The Plot on the left below shows us that up until the end of February (reported in week 11), the provisional reports lined up remarkably well. This shows us that the reporting lags are similar between 2019 and 2020. Then using data from the most recent week, we can see a sharp divergence betwene the 2019 and 2020 curves. The difference betwene these curves gives a crude estimate of excess deaths nationally, adjusted for reporting delays
...
par(mfrow=c(1,2))
for(week.test in c('11','19')){ #week for which CDCfluview is for
...
There were an estimated `r eval.sum['2019-2020'] - eval.sum['2018-2019']` more deaths in 2019 vs 2020 from March 1 through two weeks ago. In comparison, there were just `r eval.sum.comp['2019-2020'] - eval.sum.comp['2018-2019']` more deaths in 2020 vs 2019 in January and February.

## Model-based estimates
Instead, we can use regression models to estimate a seasonal baseline. We can adjust for time of year (using harmonics) and variations year to year (using dummy variables for July-June year, or linear trend). We also might want to adjust for other variables that might influence death rates, like influenza. Why would we adjust for influenza? Influenza causes deaths to increase, so to avoid mis-attributing increases in deaths to flu, we can adjust for it in the regression. On the other side, influenza rates (both hospitalizations and outpatient tests) are at historically-low levels, and this will pull the baseline down (leading to a larger estimate of excess deaths). In addition, there are reporting lags in the data, and the severity of these lags vary by state. To get an accurate picture of excess deaths it is necessary to adjust for these reporting delays. We present results from 3 different sets of analyses:

1. A model adjusting only for seasonality and year-to-year variations, with no adjustment for reporting delays

2. A model adjusting for seasonality and year-to-year variations, and adjusting for reporting delays (as estimated with a model similar to the one implemented in the NobBs package by McCough et al.)

3.A model adjusting for seasonality, year-to-year variations, and influenza activity (based on data from NREVSS), but not adjusting for reporting delays.
...
res.pi.mod3 <- format_excess_func(excess_ac_mod3,'nchs.pic' )

write.csv(res.pi.mod3, './outputs/p_i_natl_obs_expected.csv')

res.pi.nat <- res.pi.mod3[res.pi$state=='US',]

sum(res.pi.nat$unexplained.cases[res.pi.nat$mmwr_week>=10  & res.pi.nat$week_end>=as.Date('2020-01-01')])
```
...
excess_ac_mod1 <-
  excessCases(ds = analysis.data2,
              ...
              model.type='negbin',
              time.res='week')
res.pi.mod1 <- format_excess_func(excess_ac_mod1,'nchs.pic' )
...
sum.excess.deaths.range.ac.mod3 <-t(
  apply(sum.ac.excess,2,quantile,  probs=c(0.025,0.1,0.3,0.5,0.975)))

sum.excess.deaths.range.ac.mod3 <- cbind.data.frame('state'=unique(analysis.data2$state), sum.excess.deaths.range.ac.mod3)

res.ac.mod3 <- format_excess_func(excess_ac_natl_mod3,'all_deaths')

res.ac.nat.mod3 <- res.ac.mod3[res.ac.mod3$state=='US',]

yrange.pneu <- range(c(res.ac.nat.mod3$pred, res.ac.nat.mod3$obs, res.ac.nat.mod3$upi,0))

nat.range.ac.mod3 <- sum.excess.deaths.range.ac.mod3[sum.excess.deaths.range.ac.mod3$state=='US',]

formatted.ac.mod3 <- 
  paste0(round(nat.range.ac.mod3['50%'],-2),
                      '(' ,
                      round(nat.range.ac.mod3['2.5%'],-2), ',' ,
                      round(nat.range.ac.mod3['97.5%'],-2), ')'
                      )
...
```{r plot_ac_natl}
res.ac.nat.mod3 <- res.ac.mod3[res.ac.mod3$state=='US',]
plot(res.ac.nat.mod3$week_end, res.ac.nat.mod3$pred, type='l', ylim=yrange.pneu, col='gray', bty='l', ylab='All-cause Deaths', xlab='')

polygon(c(res.ac.nat.mod3$week_end, rev(res.ac.nat.mod3$week_end)), c(res.ac.nat.mod3$lpi, rev(res.ac.nat.mod3$upi)),col = rgb(0, 0, 0, alpha = 0.1), border = NA )

points(res.ac.nat.mod3$week_end, res.ac.nat.mod3$obs, type='l', col='red', bty='l')
```
...
sum.excess.deaths.range.ac.mod1 <-t(
  apply(sum.ac.excess,2,quantile,  probs=c(0.025,0.1,0.3,0.5,0.975)))

sum.excess.deaths.range.ac.mod1 <- cbind.data.frame('state'=unique(analysis.data2$state), sum.excess.deaths.range.ac.mod1)

res.ac.mod1 <- format_excess_func(excess_ac_natl_mod1,'all_deaths')

res.ac.nat.mod1 <- res.ac.mod1[res.ac.mod1$state=='US',]

yrange.pneu <- range(c(res.ac.nat.mod1$pred, res.ac.nat.mod1$obs, res.ac.nat.mod1$upi,0))

nat.range.ac.mod1 <- sum.excess.deaths.range.ac.mod1[sum.excess.deaths.range.ac.mod1$state=='US',]

formatted.ac.mod1 <- 
  paste0(round(nat.range.ac.mod1['50%'],-2),
                      '(' ,
                      round(nat.range.ac.mod1['2.5%'],-2), ',' ,
                      round(nat.range.ac.mod1['97.5%'],-2), ')'
                      )
```
 The first model (seasonality and changes over time) yields an estimate of `r if(!params$web.version){print(formatted.ac.mod1)}` excess deaths. This is *not* adjusted for reporting delays. The 3rd model, which is similar to model 1 but also adjusts for influenza estimates `r if(!params$web.version){print(formatted.ac.mod3)}` excess deaths. This is not adjusted for reporting delays, but the decline in flu activity pulls the seasonal baseline down, which explains the larger number here. Remember the simple comparison of provisional counts from 2020 and 2019 yielded a difference of `r eval.sum['2019-2020'] - eval.sum['2018-2019']` deaths, suggesting the adjusted estimates are more reliable.
...
nobbs <- read.csv('./outputs/NobBs.preds.csv')
nobbs$state <- state.abb2[match(nobbs$state, state.name2)]
nobbs <- nobbs[,c('state','death_date','obs','pred.med','pred.lcl','preds.ucl')]
names(nobbs) <-c('state','death_date','nobs.obs','nobbs.med','nobbs.lcl','nobbs.ucl')

comb1 <- merge(res.ac2, res.pi2,
               by=c('state','week_end_date'), all=T)
comb1 <- merge(comb1, nobbs,
               by.x=c('state','week_end_date'),by.y=c('state','death_date') ,all=T)

#for nobs estimates, replace with observed values in pre period
comb1$nobbs.med[is.na(comb1$nobbs.med)] <-
  comb1$all_cause_deaths[is.na(comb1$nobbs.med)]

comb1$nobbs.lcl[is.na(comb1$nobbs.lcl)] <-
  comb1$all_cause_deaths[is.na(comb1$nobbs.lcl)]

comb1$nobbs.ucl[is.na(comb1$nobbs.ucl)] <-
  comb1$all_cause_deaths[is.na(comb1$nobbs.ucl)]

nchs.covid1<-
  analysis.data2[,c('state','week_end','nchs.covid.deaths')]


comb2 <- merge(comb1,               nchs.covid1,by.x=c('state','week_end_date'), by.y=c('state','week_end'), all=T)
...
#Output time series
obs.adj1.sum.wk.q.m <- melt(obs.adj1.sum.wk.q)
obs.adj1.sum.wk.q.c <- dcast( obs.adj1.sum.wk.q.m, Var3+Var2~Var1)
names(obs.adj1.sum.wk.q.c) <- c('state','week_end','delay.adj.count.lcl','delay.adj.count.med','delay.adj.count.ucl')
write.csv(obs.adj1.sum.wk.q.c,'./outputs/delay.adjusted.counts.csv')

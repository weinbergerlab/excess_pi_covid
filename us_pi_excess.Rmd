...
The blue line shows the number of excess P&I cases, the red solid line shows the reported number of COVID-19 deaths for the same week (as compiled by the New York Times), and the dotted red line shows the reported COVID-19 deaths for weeks in which the CDC data were not yet available. Excess P&I estimates from recent weeks are adjusted for incomplete records using the "percent_complete" variable.
...
nyt<-as.data.frame(read_csv(url(urlfile)))
    ...
    #Calculate new cases/day/state from cumulative variable,
    #then sum new cased by week and state
...
nyt.c2$new.cases[is.na(nyt.c2$new.cases)] <- 0

nyt.c2 <- merge(nyt.c2, test.ds3.spl, by.x=c('state','date'), by.y=c('state','test.dates'), all=T)

nyt.c2 <- nyt.c2[ !is.na(nyt.c2$new.cases), ]
```

```{r, fig.width=8, fig.height=7}

par(mfrow=c(4,4))
y2.range.test <- range(nyt.c2$test.week.per.capita, na.rm=T)
...
## Alternate Fig 3: Instead use the JHSPH mortality data
...
par(mfrow=c(4,4))
y2.range.test <- range(jh3$test.week.per.capita, na.rm=T)
for(i in 1:length(states.cdc)){
    ds.select <- jh3[jh3$state==states.cdc[i],]
    y.range1<-range(c(ds.select$deaths, ds.select$excess_pi/ds.select$percent_complete), na.rm=T)
    ds.select$death.early <- ds.select$deaths
...
This shows that the Johns Hopkins data and NYTimes data are off by ~ 1 week in the date assignments. The Florida data records the 'date case counted', so it is possible this date is not date of death either--If this date is when the case was recognized rather than when they died, for instance.

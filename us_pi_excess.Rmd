...
last.date.obtained <- 
 ...
 pi.data <- pi.data[pi.data$week_end <=
                      (last.date.obtained-params$n.days.filter),]
...
natl.all.data <- 
  natl.all.data[,c('year','week_end', 'year_week_num',"all_deaths","number_pneumonia","total_pni" )]
...
names(nchs_covidview) <- 
  c('week_end','covid19.nchs', 'all_deaths',
...
agg.historic <- aggregate(natl.all.data.sub[,c("all_deaths","number_pneumonia","total_pni", 'covid19.nchs')], by=list('year'=natl.all.data.sub$year), FUN=sum)

natl.all.data$state <- 'NATL'
plot(natl.all.data$week_end, natl.all.data$number_pneumonia, type='l')
...
plot(natl.all.data2$week_end, natl.all.data2$pic_deaths, type='l')
points(natl.all.data2$week_end,pred, type='l', col='gray')

res.ac <-cbind.data.frame('week_end'=natl.all.data2$week_end,
                          'pred'=pred[,1,1], 'upi'=upi[,1,1], 'lpi'=lpi[,1,1],'obs'=obs[,1,1],
                      'unexplained.cases'=unexplained.cases[,1,1])

mmwr.rs <- mmwr_week(res.ac$week_end)
res.ac<- cbind.data.frame(mmwr.rs, res.ac)
#plot(res.ac$week_end, res.ac$unexplained.cases)

res.ac <- res.ac[res.ac$week_end<=as.Date('2020-03-28'),]
yrange.pneu <- range(c(res.ac$pred, res.ac$obs, res.ac$upi,0))

write.csv(res.ac, './outputs/p_i_natl_obs_expected.csv')

sum(res.ac$unexplained.cases[res.ac$mmwr_week>=10 &
                               res.ac$mmwr_week<=13 & res.ac$week_end>=as.Date('2020-01-01')])
```
...
sum(res.ac$unexplained.cases[res.ac$mmwr_week>=10 & res.ac$mmwr_week<=13 & res.ac$week_end>=as.Date('2020-01-01')])
...
polygon(c(res.ac$week_end, rev(res.ac$week_end)), c(res.ac$lpi, rev(res.ac$upi)),col = rgb(1, 1, 1, alpha = 0.1), border = NA )

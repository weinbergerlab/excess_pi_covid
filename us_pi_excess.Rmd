...
pi.data <- pi.data[pi.data$coverage_area=='state',c('week_end','percent_pni', "total_pni","all_deaths" ,'region_name')]
names(pi.data) <-c('week_end',"percent_pni",'pni_fv','all_deaths_fv','region_name')
...
natl.pi.data <- 
  aggregate(pi.data.exclude[,c("pi.fv", 'all_deaths_fv')],
...
state.pi.data <-
  pi.data[,c('week_end','week','pi.fv','all_deaths_fv','state')]
...
```{r plot_ac_natl, fig.width=6, fig.height=4}
...
nat.range.ac2$excess <- nat.range.ac2$obs - nat.range.ac2$pred

nat.range.ac2$band1 <- nat.range.ac2$pred +
  nat.range.ac2$covid.track.death

#nat.range.ac2$band2 <- nat.range.ac2$pred +
#  nat.range.ac2$excess_pic

nat.range.ac2 <- nat.range.ac2[!is.na(nat.range.ac2$pred),]

nat.range.ac2 <- nat.range.ac2[nat.range.ac2$week_start>= as.Date('2020-03-01'),]

yrange.pneu <- range(c(nat.range.ac2$pred, nat.range.ac2$obs))

plot(nat.range.ac2$week_start, nat.range.ac2$pred, type='l', ylim=yrange.pneu, col='darkgray', bty='l', ylab='All-cause Deaths', xlab='', lty=2)

points(nat.range.ac2$week_start, nat.range.ac2$obs, type='l', col='black', bty='l')

polygon(c(nat.range.ac2$week_start, rev(nat.range.ac2$week_start)), c(nat.range.ac2$pred, rev(nat.range.ac2$band1)),col =rgb(141/255,160/255,203/255, alpha = 0.9), border = NA )

# polygon(c(nat.range.ac2$week_start, rev(nat.range.ac2$week_start)), c(nat.range.ac2$band1, rev(nat.range.ac2$band2)),col = rgb(252/255,141/255,98/255, alpha = 0.9), border = NA )
# 
# polygon(c(nat.range.ac2$week_start, rev(nat.range.ac2$week_start)), c(nat.range.ac2$band2, rev(nat.range.ac2$obs)),col = rgb(102/255,194/255,165/255, alpha = 0.9), border = NA )

points(nat.range.ac2$week_start, nat.range.ac2$pred, type='l', ylim=yrange.pneu, col='black', lty=2, lwd=2)

legend('topleft',inset=0.05, 
       pch=c(15,15,15, NA, NA),
       lty=c(NA, NA, NA, 1,2), 
       lwd=c(NA, NA, NA, 1,2), 
       legend=c(  'Unexplained Excess','Pneumonia & Influenza','Reported COVID-19', 'Observed Deaths', 'Expected Deaths'),
...
```{r, table1}
nchs.tot.deaths <- aggregate(comb2[comb2$week_end_date>=count.start.date & comb2$week_end_date<=end.data.date,                           c('nchs.covid.deaths','covid.track.death',"baseline_all_cause",'all_cause_deaths'  )], by=list('state'=comb2$state[comb2$week_end_date>=count.start.date & comb2$week_end_date<=end.data.date]), FUN=sum, na.rm=T)
...
excess.combined2 <-
  excess.combined[,c('state',"all_cause_deaths","baseline_all_cause", "formatted.excess.ac",'nchs.covid.deaths', "Excess P&I&C", "Excess all-cause deaths/100,000")]

...
res.ac.nat <- merge(res.ac.nat,analysis.data2[analysis.data2$state=='US',c("nchs.covid.deaths", 'state', 'week_end')] , by=c('week_end'))
...
res.ac <- merge(res.ac,analysis.data2[,c("nchs.covid.deaths", 'state', 'week_end')] , by=c('state', 'week_end'))
...
```{r jama_fig1.plot_ac_natl, fig.width=6, fig.height=4}

covid.deaths.nat <- aggregate(test.ds3.spl[,'covid.track.death',drop=F], by=list('week_start'=test.ds3.spl$date), FUN=sum)

covid.deaths.nat$week_end <- covid.deaths.nat$week_start +days(6)

nat.range.ac2 <- merge(res.ac.nat, covid.deaths.nat, by='week_start' , all=T)

res.pi.trim <- res.pi.nat[,c('week_start','pred','obs')]
...
nat.range.ac2$covid.track.death[is.na(nat.range.ac2$covid.track.death)] <-0

nat.range.ac2$week_end <- nat.range.ac2$week_start +days(6)

stack_plot_func(nat.range.ac2)

```
Observed deaths/baseline over time
```{r}

rat1 <- nat.range.ac2$obs/nat.range.ac2$pred
plot(nat.range.ac2$week_end.x, rat1, type='l')
```
...
res.ac2$denom <-NULL
names(res.ac2) <-
  c('state','week_end_date','year','week','day','week_start_date','baseline_all_cause','baseline_all_cause_lower', 'baseline_all_cause_upper', 'all_cause_deaths','excess_all_cause_deaths', 'nchs.covid.deaths')

res.ac2 <- res.ac2[,c('state','week_end_date','week_start_date','baseline_all_cause','baseline_all_cause_lower', 'baseline_all_cause_upper', 'all_cause_deaths','excess_all_cause_deaths', 'nchs.covid.deaths')]
...
#Small cells suppressed by NCHS. in these instances, replace with Covidtrack numbers
#comb2$nchs.ct.deaths.comb <- comb2$nchs.covid.deaths

# comb2$nchs.ct.deaths.comb[comb2$nchs.covid.deaths==0 &comb2$nchs.covid.deaths>0 ] <- 
#   comb2$covid.track.death[comb2$nchs.covid.deaths==0 &comb2$nchs.covid.deaths>0 ]
...
points (st1$week_end_date ,st1$nchs.covid.deaths.y,type='l', col='red')
...
TODO: SUB IN NATIONAL ESTIMATES FROM SUMMING STATES

```{r, table1}
comb3 <- comb2[comb2$week_end_date>=count.start.date & comb2$week_end_date<=end.data.date,]

#if cells are suppressed by NCHS, sub in the covidtracking data for that week and state
comb3$corrected.nchs.covid <-comb3$nchs.covid.deaths.y
comb3$corrected.nchs.covid[comb3$nchs.covid.deaths.y==0 & comb3$covid.track.death>0] <- comb3$covid.track.death [comb3$nchs.covid.deaths.y==0 & comb3$covid.track.death>0]

nchs.tot.deaths <- aggregate(comb3[comb3$week_end_date>=count.start.date & comb3$week_end_date<=end.data.date,                           c('nchs.covid.deaths.x','covid.track.death','corrected.nchs.covid',"baseline_all_cause",'all_cause_deaths'  )], by=list('state'=comb3$state[comb3$week_end_date>=count.start.date & comb3$week_end_date<=end.data.date]), FUN=sum, na.rm=T)
...
excess.combined2 <-
  excess.combined[,c('state',"all_cause_deaths","baseline_all_cause", "formatted.excess.ac",'corrected.nchs.covid', "Excess P&I&C", "Excess all-cause deaths/100,000")]
...
all.deaths.provis.eval.comp <-
  all.deaths.provis[all.deaths.provis$Week >=2 &

...
```{r states_to_plot}
plot.states <- c('CA','TX','FL','NJ', 'NY','PA','IL','GA','MI','LA', 'MA' )
...
res.pi2 <- res.pi1
names(res.pi2) <-
  c('week_end_date','state','baseline_pi','baseline_pi_lower', 'baseline_pi_upper', 'pneumonia_influenza_covid','excess_pneumonia_influenza_covid')
...
comb2 <- merge(comb1,
               nchs.covid1,by.x=c('state','week_end_date'), by.y=c('state','week_end'))
...
comb2$rr.pic <-
  comb2$pneumonia_influenza_covid/comb2$baseline_pi
comb2$epiweek <- NA
comb2$epiweek[comb2$week<=26] <-
  comb2$week[comb2$week<=26] +26

comb2$epiweek[comb2$week>=27] <-
  comb2$week[comb2$week>=27] -26

comb2$epiyr <- comb2$year
comb2$epiyr[comb2$week<=26] <-
  comb2$year[comb2$week<=26] -1

comb2$week_end_date <-NULL
comb2.m <- melt(comb2, id.vars=c('state','epiweek','epiyr'))
...
for(i in plot.state.indices){
    y.range1<-range(c( pred/denom,obs/denom, upi/denom), na.rm=T)

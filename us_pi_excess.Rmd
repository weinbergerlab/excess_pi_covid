...
#pi.data.clean.ny
#1.6*1.2*1.07*1.04*1.02*1.01^4
pi.data.clean.ny$pull.date <- as.Date('2020-04-17')
pi.data.clean.ny$data.age <- round(as.vector(difftime(pi.data.clean.ny$pull.date,pi.data.clean.ny$week_end,  units='week')))

pi.data.clean.ny$under.report.correct <- 1
pi.data.clean.ny$under.report.correct[pi.data.clean.ny$data.age ==1] <- 1/(1.6*1.2*1.07*1.04*1.02*1.01^4)

pi.data.clean.ny$under.report.correct[pi.data.clean.ny$data.age ==2] <- 1/(1.2*1.07*1.04*1.02*1.01^4)

pi.data.clean.ny$under.report.correct[pi.data.clean.ny$data.age ==3] <- 1/(1.07*1.04*1.02*1.01^4)

pi.data.clean.ny$under.report.correct[pi.data.clean.ny$data.age ==4] <- 1/(1.04*1.02*1.01^4)

pi.data.clean.ny$under.report.correct[pi.data.clean.ny$data.age ==5] <- 1/(1.02*1.01^4)

pi.data.clean.ny$under.report.correct[pi.data.clean.ny$data.age ==6] <- 1/(1.01^4)

pi.data.clean.ny$under.report.correct[pi.data.clean.ny$data.age >=7 & pi.data.clean.ny$data.age <=10] <- 1/(1.01^4)

pi.data.clean.ny$under.report.correct <- pi.data.clean.ny$under.report.correct-0.5

excess_deaths1.ac <-
  excessCases(ds = pi.data.clean.ny,
              datevar       = "week_start",
              statevar      = "state",
              denom.var     = "under.report.correct",
              adj.flu       = "flu_pct_adj",
              #covs=c("flu_pct_adj", "flu_pct_adj_lag1", "flu_pct_adj_lag2"),
...
select.indices <- which(dates1.ac >= as.Date('2020-02-01'))
...
pred.all <- apply(pred.ac[select.indices,,1]/denom.ac[select.indices,,1],1,sum)
obs.all <- apply(obs.ac[select.indices,,1]/denom.ac[select.indices,,1],1,sum)
obs.all - pred.all

sum(obs.ac[select.indices,,1]/denom.ac[select.indices,,1]) - 
  sum(pred.ac[select.indices,,1]/denom.ac[select.indices,,1]) 
```
...
plot(dates1.ac[dates.select],excess_all_cause[dates.select,'NY',1]/denom.ac[dates.select,'NY',1]+excess_all_cause[dates.select,'NYC',1]/denom.ac[dates.select,'NY',1], type='l', bty='l', ylab='Excess deaths (N)', main='New York, incl NYC')
points(dates1[dates.select.pi],excess_pi[dates.select.pi,'NY',1]/denom.ac[dates.select,'NY',1], type='l', col='red')
...
plot(dates1.ac[dates.select],excess_all_cause[dates.select,'NJ',1]/denom.ac[dates.select,'NY',1], type='l', bty='l', ylab='Excess deaths (N)', main='New Jersey')
points(dates1[dates.select.pi],excess_pi[dates.select.pi,'NJ',1]/denom.ac[dates.select,'NY',1], type='l', col='red')
...
```{r}
##Look at backfilling rates
d1 <- readRDS('./Data/pi_mortality_state/2020_04_13_20_55.rds')
d2 <- readRDS('./Data/pi_mortality_state/2020_04_17_11_03.rds')

#Date through March 28, released April 10
d1 <- d1[ ,c('week_end','all_deaths','region_name' )]

#Data through April 10, released April 17
d2 <- d2[ ,c('week_end','all_deaths','region_name')]

names(d1) <- c('date','2020-04-10','state')
names(d2) <- c('date','2020-04-17','state')

compare.back1 <- merge(d1, d2, by=c('date','state'), all=T)


compare.m <- melt(compare.back1, id.vars  =c('date','state'))
names(compare.m) <- c('death_date','state','report_date','N_deaths')
...
library(INLA)
t2 <- as.vector(difftime(compare.m$death_date, min(compare.m$death_date),units='weeks' ))
compare.m$sin52= sin(2*pi*t2/52.1775)
compare.m$cos52= cos(2*pi*t2/52.1775)
compare.m$sin26= sin(2*pi*t2*2/52.1775)
compare.m$cos26= cos(2*pi*t2*2/52.1775)
...
compare.m$epiyr <-as.factor(epiyr)
compare.m$complete.weeks[compare.m$complete.weeks>10] <- 10 
compare.m$rev.weeks <- 10- compare.m$complete.weeks 

compare.m$complete.weeks <- 
  as.factor(compare.m$complete.weeks)
compare.m$complete.weeks <- 
  relevel(compare.m$complete.weeks, '10')
compare.m$state <- 
  as.factor(compare.m$state)
form1 <- as.formula("N_deaths ~sin52 + cos52 +sin26 +cos26 +epiyr +f(state, model='iid') + 
                    #f(rev.weeks,model='ar1') 
                    complete.weeks
                    "  )


compare.m <- compare.m[compare.m$death_date >= as.Date('2016-07-01'),]
...
summary(dat.inla)
plot(dat.inla$summary.random$rev.weeks[ ,"mean"],ylab="fitting result",type="l")
points(dat.inla$summary.random$rev.weeks$`0.025quant`, type='l', lty=2)
points(dat.inla$summary.random$rev.weeks$`0.975quant`, type='l', lty=2)

compare.back <-compare.back1
compare.back$change <- compare.back$deaths_rec_2020_04_10/compare.back$deaths_rec_2020_04_17
dates.select <- which(compare.back$date >= as.Date('2020-01-01'))
#First set of dates show 2 vs 3 weeks back
#Next shows 3 vs 4 weeks back, etc
plot(compare.back$date[dates.select], compare.back$change[dates.select])

#View(compare.back)

ave.change <- 
  aggregate(compare.back[,'change', drop=F], by=list('date'=compare.back$date), FUN=mean, na.rm=T)
plot(ave.change$change)

round(1/ave.change$change,2)

#For data from 3 weeks ago, to get correct version will be obs_t-3/(1.07*1.04*1.02*1.01^4)

#for data from 2 weeks ago:
#obs_t-2/1.2*1.07*1.04*1.02*1.01^4

#for data from 1 week ago: (get from nchs waybackmachine)
#obs_t-1/1.6*1.2*1.07*1.04*1.02*1.01^4

#Based on national All-cause deaths

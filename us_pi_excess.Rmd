...
comb2$rr.ac <-
  comb2$all_cause_deaths/comb2$baseline_all_cause

comb2$epiweek <- NA
comb2$epiweek[comb2$week.x<=26] <-
  comb2$week.x[comb2$week.x<=26] +26

comb2$epiweek[comb2$week.x>=27] <-
  comb2$week.x[comb2$week.x>=27] -26

comb2$epiyr <- NA
comb2$epiyr <- comb2$year.x
comb2$epiyr[comb2$week.x<=26] <-
  comb2$year.x[comb2$week.x<=26] -1
...
for(i in plot.state.indices){
    y.range1<-range(c( pred[i,,]/denom[i,,],obs[i,,]/denom[i,,], upi[i,,]/denom[i,,]), na.rm=T)
...
for(i in plot.state.indices){
    y.range1<-range(c( pred[i,,],obs[i,,], upi[i,,]), na.rm=T)
    if(states[i] %in% state.abb2 ){
      state.name.plot <-    
      state.name2[match(states[i],state.abb2)]
    }else{

...
#   called again?
runIfExpired <- function(storeName, f, maxage=hours(48)) {
  basepath <- "Data/"
...
```{r}
#Median of Reporting delays from NobBS
...
# ~jsonlite::read_json(url.nchs.base2, simplifyVector = T))
nchs.base2.data <-
  read.csv('./Data/nchs_base2_data/Weekly_Counts_of_Deaths_by_State_and_Select_Causes__2019-2020.csv')

nchs.base2.data <- nchs.base2.data[, c("Jurisdiction.of.Occurrence","MMWR.Year", "MMWR.Week","Week.Ending.Date", 'All.Cause' ,"flag_allcause" )]

nchs.base2.data$All.Cause <-
  as.numeric(nchs.base2.data$All.Cause)

names(nchs.base2.data) <-c('state', 'year','week','week_end','all_cause', 'flag_ac')

nchs.base.comb <-
  bind_rows(nchs.base1.data,nchs.base2.data)

nchs.base.comb$week_end <- as.Date(nchs.base.comb$week_end, '%m/%d/%Y')
...
pi.data <- merge(nchs.base.comb, pi.data, by.x=c('state','week_end'), by.y=c('region_name','week_end'), all=T)
...
state.pi.data <-
  pi.data[,c('week_end','week','pi.fv','all_cause','state')]
...
#Posterior draws from rpeorting delay
complete.prop.est.iter.df <-
  readRDS('./outputs/NobBs.complete.prop.est.iter.df.rds')
...
ac.nat.obs.state <-    
  excessExtract(ds = ds,
                syndrome = 'all_deaths',
                extract.quantity = "y")
ac.nat.obs <- ac.nat.obs.state[,dimnames(ac.nat.obs.state)[[2]]!='US',1]
...
#remove reporting delay effect from baseline
pred.iter.ac3 <- array(NA, dim=dim(pred.iter.ac2))
for(i in 1:dim(pred.iter.ac2)[2]){
  pred.iter.ac3[,i,,1] <- pred.iter.ac2[,i,,1]/denom.check[,,1]
}
dimnames(pred.iter.ac3) <- dimnames(pred.iter.ac2)


complete.prop.est.iter.df.m <-
  melt(complete.prop.est.iter.df, id.vars=c('state','n.weeks.since.death' ))

complete.prop.est.iter.df.c <- acast(complete.prop.est.iter.df.m, n.weeks.since.death~variable~state)

dimnames(complete.prop.est.iter.df.c)[[3]] <- state.abb2[match(dimnames(complete.prop.est.iter.df.c)[[3]], state.name2)]

#Sort times since death oldest to most recent
complete.prop.est.iter.df.c <- complete.prop.est.iter.df.c[rev(dimnames(complete.prop.est.iter.df.c)[[1]]),,]

#sort states to match what is in pred.iter.ac3
complete.prop.est.iter.df.c <- complete.prop.est.iter.df.c[,,dimnames(pred.iter.ac3)[[3]]]

#only keep as many iterations as are in pred.iter.ac3
complete.prop.est.iter.df.c <- complete.prop.est.iter.df.c[,1:dim(pred.iter.ac3)[2],]

#array of 1s to fill in prop. complete for earlier dates
fill.1 <- array(1, dim=c(  (dim(pred.iter.ac3)[1] - dim(complete.prop.est.iter.df.c)[1]), dim(complete.prop.est.iter.df.c)[2],                     dim(complete.prop.est.iter.df.c)[3]))

complete.prop.est.iter.df.c2 <-
  abind(fill.1,complete.prop.est.iter.df.c, along=1)

pred.iter.ac4 <- pred.iter.ac3[,,,1] * complete.prop.est.iter.df.c2

#test1<- apply(complete.prop.est.iter.df.c2,c(1,3), quantile, probs=c(0.025,0.5, 0.975))

dimnames(ac.nat.obs.state)[[1]] <-as.character(dates1)
obs.state.m <- melt(ac.nat.obs.state)
names(obs.state.m) <- c('week_start','state','one','obs')

state.ac.q <- apply(pred.iter.ac4,c(1,3), quantile , probs=c(0.025, 0.5, 0.975), na.rm=T)
dimnames(state.ac.q)[[2]] <- as.character(dates1)
state.ac.q.m <- melt(state.ac.q)
state.ac.q.c <- dcast(state.ac.q.m, Var2+Var3~Var1)
names(state.ac.q.c) <- c('week_start', 'state', 'lpi','pred','upi')
state.ac.q.c$week_start <-
  as.Date(state.ac.q.c$week_start)
stateweek <- MMWRweek(state.ac.q.c$week_start)
state.ac.q.c <- cbind.data.frame(stateweek,state.ac.q.c)
state.ac.q.c$week_end <- state.ac.q.c$week_start +6

state.ac.q.c <- merge(state.ac.q.c,obs.state.m, by=c('state','week_start' ))
state.ac.q.c$unexplained.cases <- 
  state.ac.q.c$obs - state.ac.q.c$pred


#Sum the state-level estimates to get national estimate
pred.iter.ac2.nat <-
  pred.iter.ac4[,,dimnames(pred.iter.ac4)[[3]]!='US']

pred.iter.ac2.nat.sum <- apply(pred.iter.ac2.nat,c(1,2),sum)

pred.iter.ac2.nat.q <- t(apply(pred.iter.ac2.nat.sum,1, quantile, probs=c(0.025,0.5,0.975), na.rm=T))
...
eval.indices <- which(dates1 >= params$count.start.date )
pred.iter.eval <- pred.iter.ac4[eval.indices,,]
nat.sum.pred.iter.ac <- apply(pred.iter.eval,2,sum)
...
stack_plot_func(nat.range.ac2[nat.range.ac2$state=='US.agg',], scale.plot=1.3,legend.loc='bottomleft')
...
```{r jama_im_figs2.3, fig.width=8, fig.height=3.25}
...
stack_plot_func(nat.range.ac2.spl[['NY']], scale.plot=6.8)
title("New York (exclude NYC)")

stack_plot_func(nat.range.ac2.spl[['NYC']], legend=F, scale.plot=6.8)
title("New York City")
...
stack_plot_func(nat.range.ac2.spl[['NJ']], legend=F,scale.plot=fig.scaling)
title("New Jersey")

stack_plot_func(nat.range.ac2.spl[['IL']], legend=F,scale.plot=fig.scaling)
title("Illinois")

stack_plot_func(nat.range.ac2.spl[['MI']], legend=T,scale.plot=fig.scaling)
title("Michigan")

stack_plot_func(nat.range.ac2.spl[['MA']], legend=F,scale.plot=fig.scaling)
title("Massachusetts")

```

Other states of interest
```{r jama.im.other.states, fig.width=8, fig.height=6.5}
fig.scaling=1.5
par(mfrow=c(2,2), mar=c(2,4,2,1))
stack_plot_func(nat.range.ac2.spl[['DE']], legend=F,scale.plot=fig.scaling)
title("Delaware")

stack_plot_func(nat.range.ac2.spl[['FL']], legend=F,scale.plot=fig.scaling)
title("Florida")

stack_plot_func(nat.range.ac2.spl[['KY']], legend=T,scale.plot=fig.scaling,legend.loc='bottomleft')
title("Kentucky")

stack_plot_func(nat.range.ac2.spl[['CA']], legend=F,scale.plot=fig.scaling)
title("California")
```
...
## JAMA-IM fig 4: Reported number of COVID-19 deaths,compared with the excess deaths due to pneumonia and influenza in each week, by state. 
...
```{r fig3, fig.width=6, fig.height=4}
## uses NCHS covid deaths for reported deaths
jh3.trim <- jh3[jh3$week_end_date<=end.data.date,]
  par(mfrow=c(2,2), mar=c(2,4,3,1), oma = c(3,3,0,3) + 0.1)
ts.plot.func(ds.plot=jh3.trim,   states.plot=jama.states,ylim.adj=15.0 ,death.var="covid.track.death" ,plot.order=T)
...
```{r}
#Compare covid trakc death and NCHS deaths
for(i in c('NY','LA')){
st1 <- jh3[jh3$state==i,]
...
dates.plot4 <- seq.Date(from=dates1$week_end_date[dates1$year==2019 & dates1$week==27][1], length.out=52, by='week')
...
par(mfrow=c(1,2))
for(week.test in c('11','20')){ #week for which CDCfluview is for
...
excess.base<- eval.sum.comp['2019-2020'] - eval.sum.comp['2018-2019']

excess.adj <- excess.unadj- excess.base
...
sum.excess.deaths.range.pi.adj <-t(
  apply(sum.cases.excess.adj,2,quantile,  probs=c(0.025,0.5,0.975)))

sum.excess.deaths.range.pi.adj <- cbind.data.frame('state'=unique(analysis.data2$state), sum.excess.deaths.range.pi.adj)

formatted.pic.adj <- 
  paste0(round(sum.excess.deaths.range.pi.adj[,'50%'],-1),
                      '(' ,
                      round(sum.excess.deaths.range.pi.adj[,'2.5%'],-1), ',' ,
                      round(sum.excess.deaths.range.pi.adj[,'97.5%'],-1), ')'
                      )
formatted.pic.adj <- cbind.data.frame('state'=sum.excess.deaths.range.pi.adj$state,formatted.pic.adj)
...
formatted.pic.unadj <- cbind.data.frame('state'=sum.excess.deaths.range.pi$state,formatted.pic.unadj)

summary.table.adj.unadj <- merge(formatted.pic.adj,formatted.pic.unadj, by='state')

names(summary.table.adj.unadj) <- c('State','Excess P&I&C, unadjusted for influenza', 'Excess P&I&C, adjusted for influenza')

...
```{r}
#Date whn start counting excess deaths: 2 weeks prior to first reported COVID-19 death
count.start.date <- as.Date('2020-02-09')
```
...
cdc.data <- fromJSON(file=url.cdc.covid, simplify=F)
...
for(i in plot.state.indices){
  for(j in 1:dim(pred)[3]){
    y.range1<-range(c( pred[select.indices,i,j]/denom[select.indices,i,j],obs[select.indices,i,j]/denom[select.indices,i,j], upi[select.indices,i,j]/denom[select.indices,i,j]))
...
## Supplementary Table 1: Excess deaths by week across the US, compared with official numbers from NCHS
```{r}
count.excess <-jh3[jh3$date >= as.Date(count.start.date) & jh3$date <=max(pi.data$week_end) ,]
...
count.excess4 <- count.excess4[, c("Week ending","Total P&I deaths","Excess P&I deaths","NCHS Reported COVID-19 Deaths, with pneumonia code", "NCHS Reported COVID-19 Deaths" , "All reported COVID-19 deaths (covidtracking.com)"  )]
...
pi.data.clean2$percent_complete.adj <-pi.data.clean2$percent_complete - 0.5 #subtract =0.5 because 0.5 is added in code
excess_deaths1.ac <-
  excessCases(ds = pi.data.clean2,
              datevar       = "week_start",
              statevar      = "state",
...
## Figure S2: Excess all-cause deaths for NY state vs Excess deaths due to pneumonia and influenza

```{r fig.s2, fig.width=8, fig.height=5}
...
plot(dates1,excess_all_cause[,'NY',1], type='l', bty='l', ylab='Excess deaths (N)', main='New York')
...
plot(dates1[],excess_all_cause[,'NJ',1], type='l', bty='l', ylab='Excess deaths (N)', main='New Jersey')
points(dates1,excess_pi[,'NJ',1], type='l', col='red')
legend

dates.select<- which(dates1>=as.Date('2020-02-09'))

plot(dates1[dates.select],excess_all_cause[dates.select,'NY',1], type='l', bty='l', ylab='Excess deaths (N)', main='New York')
points(dates1[dates.select],excess_pi[dates.select,'NY',1], type='l', col='red')
legend(as.Date('2016-01-01'), 300, legend=c("Excess all-cause deaths", "Excess P&I deaths"),
       col=c("black", "red"), lty=1, cex=0.8)

plot(dates1[dates.select],excess_all_cause[dates.select,'NJ',1], type='l', bty='l', ylab='Excess deaths (N)', main='New Jersey')
points(dates1[dates.select],excess_pi[dates.select,'NJ',1], type='l', col='red')
legend
...
```{r}
excess_pi[dim(excess_pi)[1],'NJ',1]/excess_all_cause[dim(excess_pi)[1],'NJ',1]

excess_pi[dim(excess_pi)[1],'NY',1]/excess_all_cause[dim(excess_pi)[1],'NY',1]
```
 
## Figure S3: NYC data
```{r nyc}
nyc.data <- pi.data.clean[pi.data.clean$state=='NYC',]
nyc.data$one <- 1
nyc.pi <-
...
plot(dates2[dates.select],nyc.excess.acm[dates.select,1,1], type='l', bty='l', ylab='Excess deaths (N)', main='New York')
...
##Table S2: Do not adjust for underreporting of deaths

...
```{r states_to_plot}
plot.states <- c('CA','TX','FL', 'NY','PA','IL','OH','GA','MI','LA' )

exclude.states <- c('WV','CT', 'NC') #no up-to-date number
```
...
natl.all.data3 <-
  natl.all.data2[!is.na(natl.all.data2$total_deaths_pic_denom),]
excess_ac_natl <-
...
jh3a <- jh3
jh3a$week_end <- jh3a$date 
jh3a <- jh3a[,c('state','week_end','total_pi',"excess_pi","covid.track.death","test.week.per.capita",'testN' )]
ac.summary.state2 <- merge(ac.summary.state, jh3a,by.x=c('state','week_end_date'), by.y=c('state', 'week_end'))

ac.summary.state2$week_end_date <-
  ac.summary.state2$week_end_date+6
write.csv(ac.summary.state2,'./outputs/state.ac.summary.csv')

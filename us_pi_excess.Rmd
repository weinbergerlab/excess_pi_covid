...
```{r}
plot.states <- c('CA','TX','FL', 'NY','PA','IL','OH','GA','WA' )
```
...
test.ds2 <- merge(test.ds2, hhs_states.cw, by='state')

test.ds2 <- aggregate(test.ds2[,c('cum.tests','census_bureau_pop_2019','cov.track.cum.death')], by=list('state'=test.ds2[, 'state_region'], 'test.dates'=test.ds2$test.dates ), FUN=sum)
...
pi.data.agg <- aggregate(pi.data[,c('total_pni','all_deaths','pct.comp.wgt')], by=list('state'=pi.data$state_region, 'week_start'=pi.data$week_start, 'week_end'=pi.data$week_end), FUN=sum)
...
## Abstract
Mortality data on deaths due to pneumonia&influenza through the week ending `r  max(dates1)+7`
...
```{r, fig.width=7.3, fig.height=5.5}
...
```{r fig2, fig.width=7.3, fig.height=5.5}
...
```{r fig3, fig.width=7.3, fig.height=5.5}
## uses covidtracking.com data for reported deaths

par(mfrow=c(4,4), mar=c(2,2,1,3))
...
count.excess3 <- count.excess2[, c('state','total_pi','excess_pi_ci' ,'covid.track.death','excess_pi_ci_INC')]

...
The blue line shows the number of excess P&I cases, the red solid line shows the reported number of COVID-19 deaths for the same week, and the dotted red line shows the reported COVID-19 deaths for weeks in which the CDC data were not yet available.
...
#nyt.c2 <- merge(nyt.c2 , pi.data[c('state', 'wk_start','percent_complete')], by.x=c('state','date') , by.y=c('state','wk_start'))
...
for(i in 1:length(states.cdc)){
    ...
    y.range1<-range(c(ds.select$new.deaths, ds.select$excess_pi), na.rm=T)
    ...
    plot(ds.select$date         ,
       ...
       points(ds.select$date         ,
       ds.select$excess_pi, type='l', col='blue', lwd=2)
    abline(h=1, col='black')
}

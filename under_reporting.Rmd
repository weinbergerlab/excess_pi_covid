...
#Model Organization
##############################################
model_spec<-textConnection(model_string_pois)
...
states.test <- unique(compare.m.alt.spl2$state)
  ...
  clusterExport(cl, c('jags.func','compare.m.alt.spl2','model_string_pois'), environment())
...
baseline.ci <- ci[baseline.index,]
excess.ci <- ci[excess.index,]
all.preds<- as.numeric(as.character(cbind(pred.means, pred.ci)))
all.preds<- matrix(all.preds,ncol=3)
index <-400:nrow(all.preds)
matplot(all.preds[400:nrow(all.preds),], type='l', lty=c(1,2,2), col=c('black','gray','gray'))
points(date.sum[400:nrow(all.preds)], type='p',pch=16 ,col='red')
points(baseline[400:nrow(all.preds)], type='l' ,col='blue')
#polygon(c(1:length(index), rev(1:length(index))), c(baseline.ci[400:nrow(all.preds),1], rev(baseline.ci[400:nrow(all.preds),2])) )

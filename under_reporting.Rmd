...
all.reports <- rbind.data.frame(wk11,wk12,wk13,wk15, wk16, wk17, wk18)
...
complete.wk.iter <- apply(prop.report.wk.iter, c(1,3),cumsum)
dimnames(complete.wk.iter)[[1]] <- 2:(dim(complete.wk.iter)[1]+1)
saveRDS(complete.wk.iter,'./outputs/NobBs.complete.iters.csv')
```
...
ds.plot <- ds.plot[ds.plot$death_date>=as.Date('2020-03-01'),]
...
points(ds.plot$death_date,ds.plot$obs, type='p',pch=16 ,col='red')
polygon(c(ds.plot$death_date, rev(ds.plot$death_date)), c(ds.plot$pred.lcl, rev(ds.plot$preds.ucl)), col=rgb(0,0,1,alpha=0.1) )
}

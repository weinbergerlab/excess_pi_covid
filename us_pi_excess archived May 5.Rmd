...
nrevvs.state <- readRDS('./Data/nrevss_state/2020_05_05_15_56.rds')
...
#  nrevvs_hhs <- runIfExpired('nrevss_hhs', ~cdcfluview::who_nrevss(region = c("hhs")))
  nrevvs_hhs<-readRDS('./Data/nrevss_hhs/2020_05_05_15_56.rds')
...
nrevvs.natl <- readRDS('./Data/nrevss_national/2020_05_05_15_56.rds')
...
pi.data <- readRDS('./Data/pi_mortality_state/2020_05_05_15_56.rds')
...
cdc.data<- readRDS('./Data/cdc_covid_data/2020_05_06_10_11.rds')
...
most.recent <- min(cdc.data$data_as_of)
...
json_data<- readRDS('./Data/covidtracking_states_daily/2020_05_05_15_57.rds')
...
sum(res.pi.nat$unexplained.cases[res.pi.nat$mmwr_week>=10 &                              res.pi.nat$mmwr_week<=16 & res.pi.nat$week_end>=as.Date('2020-01-01')])
```
...
comb2.c.all <- acast(comb2.m, state~epiweek~epiyr~variable )
...
### Figure 1: Selected states only. Observed weekly death rate due to any cause vs seasonal baseline (+/-95% Prediction Interval) 
 The black line shows the observed proportion of deaths that were due to any cause per week. The red line and shaded area represent the 95% Prediction Interval. The latest P&I data is for the week ending `r  max(dates1)+6`.


```{r fig1.select, fig.width=6, fig.height=4}
par(mfrow=c(2,2), mar=c(2,4,2,1))
comb2.c <- comb2.c.all
comb2.c <- comb2.c[state.plot2,,,]
rr <- t(comb2.c[,,'2019','rr.ac'])
rr <- rr[complete.cases(rr),]
plot.state.rank <- cbind.data.frame(state.index=1:dim(rr)[2],state.rank= rank(-rr[dim(rr)[1],]))
plot.state.rank <- plot.state.rank[order(plot.state.rank$state.rank),]
...
### selected states fig 2
```{r fig2.select, fig.width=6, fig.height=4}
par(mfrow=c(2,2), mar=c(2,4,2,1))
comb2.c <- comb2.c.all
comb2.c <- comb2.c[state.plot2,,,]
...
### fig 3 alt
```{r fig3.select, fig.width=6, fig.height=4}
## uses NCHS covid deaths for reported deaths

jh3.alt <- jh3[jh3$state %in% c(state.plot2),]
  par(mfrow=c(2,2), mar=c(2,4,3,1), oma = c(3,3,0,3) + 0.1)
ts.plot.func(ds.plot=jh3.alt,   states.plot=state.plot2,ylim.adj=15.0 ,death.var="nchs.covid.deaths" , plot.order=T)
...
ili.data <- readRDS('./Data/ilinet_state/2020_05_05_15_57.rds')
...
```{r}
rr.state.pic <- t(comb2.c.all[,,'2019','rr.pic'])
rr.state.ac <- t(comb2.c.all[,,'2019','rr.ac'])
...
```{r fig4.alt, fig.width=6, fig.height=2.5}
common.states <- 
  intersect(state.plot2, colnames(rr.ili2) )
...
obs.unadj$time.since.death <- ceiling(difftime(as.Date(params$data.vintage), obs.unadj$week_end,units='weeks'))
...
excess.adj <- obs.adj1.sum.c - sum.pred.iter.unadj[,,1]
excess.adj.q <- t(apply(excess.adj,2, quantile, probs=c(0.025, 0.5,0.975)))

excess.unadj <- obs.unadj1.sum.c - sum.pred.iter.unadj[,,1]
...
names(summary.table.adj.unadj) <- c('State', 'Excess deaths, adjusted for influenza','Excess deaths, unadjusted for influenza', 'Excess deaths, adjusted for reporting delay,n not influenza')
...
#Evaluation of reporting Delays
```{r, eval=F}
...
```{r}
week.test <- '18' #week for which CDCfluview is for
...
write.csv(provis.data.c1, './outputs/provisional_deaths_reported_in_wk18.csv')
...
matplot(all.deaths.provis$epiwk ,all.deaths.provis[,c('2018-2019','2019-2020')], type='l', bty='l',col=cols1, main='Provisional deaths, week 17 FluView', lty=1:4, ylim=c(0, max(all.deaths.provis[,c('2018-2019','2019-2020')]) ), xaxt='n'   , ylab='Provisional Deaths')
...
all.deaths.provis.eval.comp <-
  all.deaths.provis[all.deaths.provis$Week >=4 &

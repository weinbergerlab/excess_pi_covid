...
for(i in plot.state.indices){
  ...
  ave.range <- c(-ave_pi*0.2, ave_pi*0.7)
 ...
 date.ci <- ds.select$date[1:length(cl.min)]
  ...
  points(ds.select$date, scale.factor.test*ds.select$test.week.per.capita, type='l', lty=3, lwd=1, col='gray', yaxt='n',xaxt='n', ylab='', xlab='')
  
  axis(side=4,at=(c(0,2,4,6)*(scale.factor.test)) , labels=c(0,2,4,6), col.ticks='gray', col='gray', col.lab='gray' ,col.axis='gray')
  mtext("Tests/1000", side=4, line=2, col='gray', cex=0.75)
  text(as.Date('2020-02-06'), ave.range[2]*0.9,state.name.plot, pos=4, cex=0.85)

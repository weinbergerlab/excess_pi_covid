...
## WARNINGS
The CDC fluview data do not seem to be updated over the course of the week (updated friday), while the COVIDview data are updated daily. As a result, it is NOT ok to just sum the pneumonia counts from fluview and subtract from the cumulative total in fluview to get an estimate for most recent week--this would lead to too many deaths being attributed to most recent week. This is only an issue if using a correction factor for under-reporting and trying to look at the trajectories. If just looking at the overall deaths since Feb 1 and not putting on a specific week, it doesn't matter if the deaths are attributed to 1 week ago, 2 weeks ago, etc. Relying on data pulled on mid-day friday would help mitigate this.
...
```{r}
#Date when start counting excess deaths: 2 weeks prior to first reported COVID-19 death
...
cdc.data.df <- lapply(cdc.data, function(play) {
  data.frame(matrix(unlist(play), ncol=8, byrow=T))
  })
cdc.data.df <- do.call(rbind, cdc.data.df)
colnames(cdc.data.df) <- names(cdc.data[[1]])
rownames(cdc.data.df) <- NULL

cdc.summary.wk <- cdc.data.df[cdc.data.df$group=='By week',]
cdc.summary.wk$wk_end <- as.Date(cdc.summary.wk$indicator, "%m/%d/%Y")
cdc.summary.wk <- cdc.summary.wk[!is.na(cdc.summary.wk$wk_end),]


cdc.summary.state <- cdc.data.df[cdc.data.df$group=='By state',]

cdc.summary.state[,-c(1:2)] <- apply(cdc.summary.state[,-c(1:2)],2, function(x) as.numeric(as.character(x)) )

cdc.summary.state$covid.pct.pneum <- cdc.summary.state$pneumonia_and_covid_deaths/cdc.summary.state$covid_deaths

cdc.summary.state$covid_non_j12_18 <- 
  cdc.summary.state$covid_deaths - 
      cdc.summary.state$pneumonia_and_covid_deaths
cdc.summary.state$state <- state.abb[match(cdc.summary.state$indicator, state.name)]
...
```{r}
#The fluview data go unti 2 weeks ago. But the NCHS data go until 1 week ago (cumulative).We can subtract the cumulative numbers from nchs from the cumulative numbers for our data to get estimate for total death and P&I for last week. This is not totally kosher if trying to estimate weekly death rates, because many of the deaths in that difference should be backfilled to earlier weeks. However, when evaluating cumulatively, this doesn't matter

pi.data.spl <- 
  split( pi.data, pi.data$region_name)

#sum since week ending feb 1 2020
pi.data.clean2.spl.cum <- t(sapply(pi.data.spl, function(x){
  apply(x[x$week_end >=
    as.Date('2020-02-01'),c('all_deaths','number_pneumonia')],
    2,sum, na.rm=T)
  }, 
    simplify='array' 
  )
...
pi.data.clean2.spl.cum <- as.data.frame(pi.data.clean2.spl.cum)
names(pi.data.clean2.spl.cum) <- c('all_deaths_2wklag','pneu_death_2wk_lag')

pi.data.clean2.spl.cum$state <- row.names(pi.data.clean2.spl.cum)
row.names(pi.data.clean2.spl.cum) <-NULL

pi.data.clean2.spl.cum$state2 <-   
 state.abb[match(pi.data.clean2.spl.cum$state, state.name)]
pi.data.clean2.spl.cum$state2[pi.data.clean2.spl.cum$state =='New York City'] <- 'NYC'


cdc.summary.state$state <-  
      as.character(cdc.summary.state$indicator)

update.pi <- merge(  cdc.summary.state, pi.data.clean2.spl.cum,by.x='state', by.y='state')

update.pi$total_deaths_latest_wk <- update.pi$total_deaths -
    update.pi$all_deaths_2wklag

update.pi$pneu_deaths_latest_wk <- update.pi$pneumonia_deaths -
    update.pi$pneu_death_2wk_lag

update.pi2 <- update.pi[,c('state','pneu_deaths_latest_wk','total_deaths_latest_wk',"percent_expected_deaths" )]
update.pi2$week_start <- as.Date(max(pi.data$week_start)) + weeks(1)
update.pi2$week_end <- as.Date(max(pi.data$week_end)) + weeks(1)
names(update.pi2) <- c('region_name', 'number_pneumonia', 'all_deaths', 'percent_complete','week_start', 'week_end')

pi.data <-bind_rows(update.pi2,pi.data)
...
pi.data.agg <- aggregate(pi.data[,c("number_pneumonia" ,'all_deaths','pct.comp.wgt')], by=list('state'=pi.data$state_region, 'week_start'=pi.data$week_start, 'week_end'=pi.data$week_end), FUN=sum, na.rm=T)

pi.data.agg$percent_complete <- 
  pi.data.agg$pct.comp.wgt/pi.data.agg$all_deaths

spl1<-split(pi.data.agg, pi.data.agg$state)
min.state <- lapply(spl1, function(x){ x$miss.x<-min(x$number_pneumonia, na.rm=T)
return(x)
                })
...
excess_deaths1.adjusted <-
  excessCases(ds = pi.data.clean2,
              ...
              use.syndromes = c("number_pneumonia"),
              extrapolation.date = "2020-02-01",
              time.res='week')
```
...
syndrome.select <- 'number_pneumonia' #which syndrome do you want to plot?
...
cdc.summary.state.agg <- cdc.summary.state
cdc.summary.state.agg$state <- state.abb[match(cdc.summary.state.agg$state , state.name)]
 cdc.summary.state.agg$state[cdc.summary.state.agg$indicator=='New York City'] <- 'NY'

 cdc.summary.state.agg <- aggregate(cdc.summary.state.agg[,'covid_non_j12_18'], by=list('state'=cdc.summary.state.agg$state), FUN=sum)
 names(cdc.summary.state.agg) <- c('state', 'covid_non_j12_18')
cdc.summary.state.agg <- 
  merge(cdc.summary.state.agg, hhs_states.cw, by='state')

cdc.summary.state.agg <-
  aggregate(cdc.summary.state.agg[,'covid_non_j12_18'], 
            by=list('state'=cdc.summary.state.agg$state_region), 
            FUN=sum)
 names(cdc.summary.state.agg) <- c('state', 'covid_non_j12_18')
 
count.excess2 <- merge(count.excess2, cdc.summary.state.agg, by='state')


count.excess2$exces_ci_ucl <- round(count.excess2$excess_pi_adj +count.excess2$covid_non_j12_18 + 1.96*sqrt(count.excess2$excess_pi_var_ADJ))

count.excess2$exces_ci_lcl <- round(count.excess2$excess_pi_adj +count.excess2$covid_non_j12_18 - 1.96*sqrt(count.excess2$excess_pi_var_ADJ))

count.excess2$exces_ci_ucl_INC <- (count.excess2$excess_pi_adj+count.excess2$covid_non_j12_18 + 1.96*sqrt(count.excess2$excess_pi_var_ADJ))/count.excess2$census_bureau_pop_2019*1000000

count.excess2$exces_ci_lcl_INC <- (count.excess2$excess_pi_adj +count.excess2$covid_non_j12_18- 1.96*sqrt(count.excess2$excess_pi_var_ADJ))/count.excess2$census_bureau_pop_2019*1000000

count.excess2$exces_pi_INC <- (count.excess2$excess_pi_adj + count.excess2$covid_non_j12_18)/count.excess2$census_bureau_pop_2019*1000000

count.excess2$excess_pi_ci <- paste0(round(count.excess2$excess_pi_adj +count.excess2$covid_non_j12_18), '(',  count.excess2$exces_ci_lcl, ', ', count.excess2$exces_ci_ucl,')'  ) 
...
count.excess3 <- count.excess2[, c('state','total_pi_adj', "excess_pi_adj" ,'covid_non_j12_18','excess_pi_ci' ,'covid.track.death','excess_pi_ci_INC')]

names(count.excess3) <-c('State','Total P&I deaths', 'Excess P&I deaths','Non-J12-J18 COVID deaths', 'Combined Excess +COVID', 'Reported COVID-19 deaths','Excess deaths/1,000,000 people')
last.date.format<-max(pi.data$week_end)
last.date.format<-format(last.date.format,
                         "%b %d, %Y")
count.excess3$`Excess P&I deaths` <-
    round(count.excess3$`Excess P&I deaths`)

htmlTable(count.excess3, caption=paste0('Observed and Excess deaths due to pneumonia & influenza, and COVID-19, from February 1, 2020 through ', last.date.format), rnames=F)
```
...
count.excess$excess_pi_adj <- 
  count.excess$excess_pi/count.excess$percent_complete

count.excess$total_pi_adj <- 
  count.excess$total_pi/count.excess$percent_complete

count.excess$excess_pi_var_ADJ <- 
  count.excess$excess_pi_var/count.excess$percent_complete
...
#count.excess4 <- 
#  merge(count.excess4, cdc.summary.state.agg,by='state' )

count.excess4 <- count.excess4[, c("Week ending","Total P&I deaths","Excess P&I deaths", "All reported COVID-19 deaths (covidtracking.com)"  )]

htmlTable(count.excess4, caption=paste0('Table S1: Comparison of data sources: Observed and Excess deaths due to pneumonia & influenza, and COVID-19, from February 9, 2020 through ', last.date.format), rnames=F)
...
#sum(cdc.summary.state.agg$covid_non_j12_18)

count.excess.all$TOTAL_BURDEN <-
  paste0(round(count.excess.all$excess_pi_adj+
                 sum(cdc.summary.state.agg$covid_non_j12_18)),
         '(',  
         count.excess.all$exces_ci_lcl+
           sum(cdc.summary.state.agg$covid_non_j12_18), 
         ', ', 
         count.excess.all$exces_ci_ucl+
           sum(cdc.summary.state.agg$covid_non_j12_18),')'  ) 
...
pred.ac.var <-  excessExtract(ds = excess_deaths1.ac,
                syndrome = syndrome.select,
                extract.quantity = "pred.var")
...
These time series plots are not necesarily useful..the spike in the last week is because these are deaths that need to be backfilled (when the databases are updated, they will be assigned to their correct week). So we should only look at these in aggregate
...
```{r}
#Sum the excess
sum.indices <-which(dates1.ac >= as.Date('2020-01-26'))

sum.pred.ac <- 
  apply(pred.ac[,,1],2, function(x) sum(x[sum.indices]))

sum.obs.ac <-
  apply(obs.ac[,,1],2, function(x) sum(x[sum.indices]))

sum.pred.var <-
  apply(pred.ac.var[,,1],2, function(x) sum(x[sum.indices]))
 
excess.ac <-  
  sum.obs.ac - sum.pred.ac

excess.ac.ucl <- 
  round( sum.obs.ac -sum.pred.ac + 1.96*sqrt(sum.pred.var))

excess.ac.lcl <- 
  round( sum.obs.ac -sum.pred.ac - 1.96*sqrt(sum.pred.var))


excess.ci <- cbind.data.frame('state'=names(excess.ac),
 'Predicted'=paste0(round(excess.ac), ' (', excess.ac.lcl,',',excess.ac.ucl,')' )) 
...
htmlTable(count.excess3, caption=paste0('Table S3. Observed and Excess deaths due to pneumonia & influenza, and COVID-19, from February 9, 2020 through ', last.date.format), rnames=F)
```
...
htmlTable(compare.excess, caption=paste0('Excess deaths due to pneumonia & influenza from February 9, 2020 through ', last.date.format, ' with or without adjustment for delayed reporting'), rnames=F)
```
